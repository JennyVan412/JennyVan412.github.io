...
function switchModes(){
  ...
  if (isDark){
    ...
    // My attempt at trying to make a color changing background for the button, more are elaborated in the style file. 
    // I wanted the color green for the background of dark mode as it would make the black letters pop out more and vice versa for the other color choice which is purple.
    document.getElementById("mode-background").backgroundColor = ("--col-04");
  } else {
    isDark = true;
    document.documentElement.style.setProperty("--col-01", lightCol);
    document.documentElement.style.setProperty("--col-02", darkCol);
    document.getElementById("mode-button").innerHTML = "Light Mode";
    document.getElementById("mode-background").backgroundColor = ("--col-03");
  }
}